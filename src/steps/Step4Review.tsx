...
const ReviewRow = ({
  ...
  <div className="flex justify-between items-start gap-2 py-2.5 border-b border-slate-100 last:border-0">
    <span className="text-sm text-slate-500 flex-shrink-0 max-w-[45%]">
      {label}
    </span>
    <span className="text-sm text-slate-800 font-medium text-right break-all">
      {value || empty}
    </span>
  </div>
);
  ...
  return (
    ...
    >
      ...
      <ReviewSection title="Purchase Preferences" step={2} onEdit={goToStep}>
        <ReviewRow
          label="Purchase Type"
          value={PURCHASE_LABELS[step2.purchaseType]}
        />
        <ReviewRow
          label="Vehicle Type"
          value={VEHICLE_LABELS[step2.vehicleType]}
        />
        <ReviewRow
          label="Budget"
          value={
            step2.budgetMin && step2.budgetMax
              ? `$${Number(step2.budgetMin).toLocaleString()} – $${Number(step2.budgetMax).toLocaleString()}`
              : undefined
          }
        />
        {step2.purchaseType === "financing" && (
          <>
            <ReviewRow
              label="Credit Score"
              value={CREDIT_LABELS[step2.creditScoreRange]}
            />
            <ReviewRow
              label="Employment"
              value={EMPLOYMENT_LABELS[step2.employmentStatus]}
            />
          </>
        )}
        {step2.vehicleType === "ev" && (
          <ReviewRow
            label="Home Charging"
            value={step2.homeCharging === "yes" ? "Yes" : "No"}
          />
        )}
      </ReviewSection>
      ...
      <ReviewSection title="Contact & Schedule" step={3} onEdit={goToStep}>
        <ReviewRow
          ...
          value={CONTACT_LABELS[step3.preferredContact]}
        />
        ...
        <ReviewRow
          label="Test Drive"
          value={
            step3.testDriveDate ? formatDate(step3.testDriveDate) : undefined
          }
          empty="Not scheduled"
        />
      </ReviewSection>
